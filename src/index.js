...
const initHome = () => {
    ...
    appendElem(content, "p", ['slight-header'], "Personal Moodboard");
    appendElem(content, "p", [], formatDate());
    const hiBlock = appendElem(content, "div", ["hi-block"]);
    appendElem(hiBlock, "p", ["slight-header"], "Hi, Billy!" );
    appendElem(hiBlock, "p", ["gray-text"], "Since we won't see each other for a long time, ");
    appendElem(hiBlock, "p", ["gray-text"], "I hope you are doing well.");
    ...
    appendElem(addTask, "p", ['task-prompt'], "Another word of shit ?");
    const taskNameInput = appendElem(addTask, "input", ['task-name-input']);
    const addTaskButton = appendElem(addTask, "div", ['action-button'], "Dump In");
...
const initTest = () => {
    const content = clearContent();
    appendElem(content, "p", ['big-header'], "Readme");
    appendElem(content, "p", ['gray-text'], "This website is dedicated to Christ Billy Prakoswa");
    appendElem(content, "p", ['gray-text'], "Features :");
    appendElem(content, "p", ['gray-text'], " ");

    appendElem(content, "p", ['gray-text'], "- View inspirational messages to keep your spirits up, and add your own at any time");
    appendElem(content, "p", ['gray-text'], "- Each day take the time to write down what's on your mind in a safe environment");
    appendElem(content, "p", ['gray-text'], "- Track your emotions over time to discover trends in your mental health and become aware in your own mind");
    appendElem(content, "p", ['gray-text'], "- Automatically generate a cloud of your most common thoughts and feelings to get a better understanding of yourself");
    appendElem(content, "p", ['gray-text'], " ");
    appendElem(content, "p", ['gray-text'], "I think he's struggling now. But I believe he can solve it in his own way. So, I tried to create this website to support him tracking, reflecting, and findiong out the best way for himself."
    );
...
const initNavigation = () => {
    const buttons = ['home', 'graph', 'word-cloud', 'about'];
...
const initProfile = (username) => {
    ...
    picture.setAttribute('src', 'img/' + 'billy.jpg');
